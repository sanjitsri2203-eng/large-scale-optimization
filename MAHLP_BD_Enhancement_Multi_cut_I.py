...
N = list(range(1, n + 1))
A = [(i, j, k, m) for i in N for j in N for k in N for m in N]
D = [(i, j) for i in N for j in N]
...
# eta[i] approximates routing cost for all flows starting from origin i.
eta_var = {i: master.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"eta_{i}") for i in N}
master.update()

master.setObjective(quicksum(eta_var[i] for i in N), GRB.MINIMIZE)
...
print("\nMAHLP Benders Decomposition - Multi-cut Strategy I")
...
while (Upper_bound - Lower_bound > tolerance) and (iteration < max_iter):
    ...
    if sub.status == GRB.OPTIMAL:
        ...
        # Multi-cut Strategy I:
        # Add one optimality cut for each origin node i.
        Cut_rhs = {}

        for i in N:
            constant_i = sum(pi_val[(i, j)] for j in N if j != i)
            coefficient_i = {h: 0.0 for h in N}

            for j in N:
                if i != j:
                    for k in N:
                        for m in N:
                            coefficient_i[k] += lambda_val[(i, j, k, m)]
                            coefficient_i[m] += mu_val[(i, j, k, m)]

            Cut_rhs[i] = constant_i - quicksum(coefficient_i[h] * y_vars[h] for h in N)

        master.addConstrs(
            (eta_var[i] >= Cut_rhs[i] for i in N),
            name=f"BendersCut_{iteration}"
        )
        master.update()
        optimal_cut_count += len(N)

    elif sub.status in (GRB.INF_OR_UNBD, GRB.UNBOUNDED):
        print("Subproblem infeasible or unbounded! Adding feasibility cuts.")
        ...
        if sub.status == GRB.UNBOUNDED:
            ...
            for i in N:
                ray_constant_i = sum(ray_pi[(i, j)] for j in N if j != i)
                ray_coefficient_i = {h: 0.0 for h in N}

                for j in N:
                    if i != j:
                        for k in N:
                            for m in N:
                                ray_coefficient_i[k] += ray_lambda[(i, j, k, m)]
                                ray_coefficient_i[m] += ray_mu[(i, j, k, m)]

                Feas_rhs[i] = ray_constant_i - quicksum(
                    ray_coefficient_i[h] * y_vars[h]
                    for h in N
                )

            master.addConstrs(
                (Feas_rhs[i] <= 0 for i in N),
                name=f"FeasCut_{iteration}"
            )
            master.update()
            feasibility_cut_count += len(N)
        else:
    ...
    # ======================================================
    # 4.2 Solve Master Problem after adding the new cuts
    # ======================================================
