...
# eta[i,j] approximates routing cost for OD pair (i,j).
eta_var = {
...
master.setObjective(quicksum(eta_var[(i, j)] for (i, j) in D_set), GRB.MINIMIZE)
...
print("\nMAHLP Benders Decomposition - Multi-cut Strategy II")
...
while (Upper_bound - Lower_bound > tolerance) and (iteration < max_iter):
    ...
    if sub.status == GRB.OPTIMAL:
        ...
        # Multi-cut Strategy II:
        # Add one optimality cut for each OD pair (i,j).
        Cut_rhs = {}

        for (i, j) in D_set:
            coefficient_ij = {h: 0.0 for h in N}

            for k in N:
                for m in N:
                    coefficient_ij[k] += lambda_val[(i, j, k, m)]
                    coefficient_ij[m] += mu_val[(i, j, k, m)]

            Cut_rhs[(i, j)] = (
                pi_val[(i, j)]
                - quicksum(coefficient_ij[h] * y_vars[h] for h in N)
            )

        master.addConstrs(
            (eta_var[(i, j)] >= Cut_rhs[(i, j)] for (i, j) in D_set),
            name=f"BendersCut_{iteration}"
        )
        master.update()
        optimal_cut_count += len(D_set)
        ...
        if sub.status == GRB.UNBOUNDED:
            ...
            for (i, j) in D_set:
                ray_coefficient_ij = {h: 0.0 for h in N}

                for k in N:
                    for m in N:
                        ray_coefficient_ij[k] += ray_lambda[(i, j, k, m)]
                        ray_coefficient_ij[m] += ray_mu[(i, j, k, m)]

                Feas_rhs[(i, j)] = (
                    ray_pi[(i, j)]
                    - quicksum(ray_coefficient_ij[h] * y_vars[h] for h in N)
                )

            master.addConstrs(
                (Feas_rhs[(i, j)] <= 0 for (i, j) in D_set),
                name=f"FeasCut_{iteration}"
            )
            master.update()
            feasibility_cut_count += len(D_set)
        else:
...
print("Sum of eta values:", sum(eta_var[(i, j)].X for (i, j) in D_set))
